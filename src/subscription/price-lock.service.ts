...
export class PriceLockService {
  ...
  async createQuote(storeId: string, periodMonths: 1 | 6 | 12, currency: 'USDC' | 'XLM'): Promise<PriceLock> {
    ...
    let amountInCurrency = baseUsdc;
    let xlmRate: number | null = null;
    if (currency === 'XLM') {
      xlmRate = await this.oracle.getXlmUsd();
      amountInCurrency = parseFloat(((baseUsdc / xlmRate) * 1.02).toFixed(7));
    }
